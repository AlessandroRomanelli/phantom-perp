...
from agents.risk.limits import limits_for_portfolio
...
class TestLimitsForPortfolio:
    def test_portfolio_a_defaults(self) -> None:
        limits = limits_for_portfolio(PortfolioTarget.A, DEFAULT_CONFIG)
    ...
    def test_portfolio_b_defaults(self) -> None:
        limits = limits_for_portfolio(PortfolioTarget.B, DEFAULT_CONFIG)
    ...
    def test_a_and_b_have_different_limits(self) -> None:
        a = limits_for_portfolio(PortfolioTarget.A, DEFAULT_CONFIG)
        b = limits_for_portfolio(PortfolioTarget.B, DEFAULT_CONFIG)
    ...
    def test_config_cannot_exceed_hard_leverage_cap_a(self) -> None:
        ...
        config = {
            "risk": {"portfolio_a": {"max_leverage": 10.0}},
        }
        limits = limits_for_portfolio(PortfolioTarget.A, config)
    ...
    def test_config_cannot_exceed_hard_leverage_cap_b(self) -> None:
        ...
        config = {
            "risk": {"portfolio_b": {"max_leverage": 5.0}},
        }
        limits = limits_for_portfolio(PortfolioTarget.B, config)
        assert limits.max_leverage == MAX_LEVERAGE_PORTFOLIO_B

    def test_config_cannot_lower_liquidation_distance_floor(self) -> None:
        """Config sets 3% liq distance, but hard floor for A is 8%."""
        config = {
            "risk": {"portfolio_a": {"min_liquidation_distance_pct": 3.0}},
        }
        limits = limits_for_portfolio(PortfolioTarget.A, config)
        assert limits.min_liquidation_distance_pct == PORTFOLIO_A_MIN_LIQUIDATION_DISTANCE_PCT
    ...
    def test_empty_config_uses_hard_caps(self) -> None:
        limits = limits_for_portfolio(PortfolioTarget.A, {})
        assert limits.max_leverage == MAX_LEVERAGE_GLOBAL
        assert limits.min_liquidation_distance_pct == PORTFOLIO_A_MIN_LIQUIDATION_DISTANCE_PCT
