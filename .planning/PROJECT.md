...
## Current Milestone: v1.4 Forensic Audit Fixes

**Goal:** Fix structural profitability issues identified by the 5-agent forensic audit — eliminate bugs causing financial loss, recalibrate sizing/execution to overcome fee drag, and fix corrupted data inputs degrading signal quality.

**Target features:**
- Tier 1 (Safety): Fix double execution, wire daily P&L into risk, serialize idea metadata, restore MAX_LEVERAGE_GLOBAL, fix Route B credentials
- Tier 2 (Profitability): Reduce conviction_power, convert SL to STOP_LIMIT, fix bar_volumes, fix ADX NaN bug, add fee-adjusted signal filter
- Tier 3 (Robustness): Cross-instrument correlation check, Bollinger ddof fix, realistic paper sim, equity HWM drawdown, index price sourcing, BTC notional cap
...
- [ ] Fix paper mode positions deserialization in risk, reconciliation, and monitoring agents
- [ ] Fix execution dedup set.pop() → FIFO eviction
- [ ] Fix reduce_only boolean parsing across all deserialization points
- [ ] Centralize serialization/deserialization into shared libs module
- [ ] Add Redis AUTH configuration and security hardening
- [ ] Replace dashboard silent exception handlers with structured logging
- [ ] Add test coverage for libs/messaging, libs/portfolio/router, risk submodules
- [ ] Add Redis PEL cleanup (XAUTOCLAIM) for crashed consumer recovery
...
- A vs B portfolio comparison in tuner — tuner targets Portfolio A only
- Real-time parameter adjustment — daily cadence is sufficient, avoids overfitting

## Context

Portfolio A is currently -$204 while Portfolio B is +$120. A codebase audit on 2026-04-08 identified critical bugs (paper mode positions bypass, dedup eviction, boolean parsing), security gaps (unauthenticated Redis, Docker socket exposure), and significant test coverage holes in core infrastructure (messaging, portfolio routing, risk submodules). This milestone addresses the highest-priority findings before resuming feature work.

## Constraints

- **Architecture**: Fixes must not change the SignalStrategy/StandardSignal contract or Redis Streams message format (backward compatible)
- **Risk**: Risk guardrails must be strengthened, never weakened — paper mode must match live mode guardrail behavior
- **Compatibility**: Centralized deserialization must be a drop-in replacement — no agent behavior changes beyond bug fixes
- **Security**: Redis AUTH must not break existing agent connectivity — phased rollout with env var config
- **Testing**: New tests must use existing test infrastructure (pytest, respx, fakeredis, AsyncMock)
...
| Claude advises, code applies | AI reasons about performance but code enforces hard bounds — no unbounded AI writes | — v1.2 |
| YAML rewrite over Redis overrides | Auditable, simple, git-diffable parameter changes | — v1.2 |
| Daily tuning cadence | Avoids overfitting from hourly micro-adjustments | — v1.2 |
