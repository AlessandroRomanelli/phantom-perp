#!/usr/bin/env python
"""Query post-tuner OBI BTC fills and print P&L metrics.

Connects to the production PostgreSQL database, fetches fills attributed to
the orderbook_imbalance strategy for a given instrument since a configurable
start date, and prints key performance metrics.

Usage (from project root, venv active):
    DATABASE_URL=postgresql://... python scripts/query_obi_btc.py
    DATABASE_URL=postgresql://... python scripts/query_obi_btc.py --since 2025-01-01
    DATABASE_URL=postgresql://... python scripts/query_obi_btc.py --instrument ETH-PERP-INTX
"""
...
logger = structlog.get_logger("query_obi_btc")


async def main() -> None:
    parser = argparse.ArgumentParser(
        description="Query post-tuner OBI BTC fills and print P&L metrics."
    )
    parser.add_argument(
        "--since",
        metavar="YYYY-MM-DD",
    ...
    if not database_url:
        print(
            ...
            "python scripts/query_obi_btc.py",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if not fills:
        print(
            f"No fills found for orderbook_imbalance / {args.instrument} "
            f"since {since_dt.date().isoformat()}."
        )
        return

    logger.info("query_obi_btc_fills_fetched", fill_count=len(fills))

    metrics_map = compute_strategy_metrics(fills, min_trades=1)
    key = ("orderbook_imbalance", args.instrument)
    metrics = metrics_map.get(key)

    if metrics is None:
        print(
            f"Insufficient data for metrics: {len(fills)} fill(s) found but no closed "
            f"round-trips for orderbook_imbalance / {args.instrument} since "
    ...
    print(
        f"\n{'='*60}\n"
        f"OBI {args.instrument} — Post-Tuner P&L Summary\n"
        f"Since: {since_dt.date().isoformat()}\n"
        f"{'='*60}\n"
