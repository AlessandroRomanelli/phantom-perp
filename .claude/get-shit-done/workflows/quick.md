...
```bash
INIT=$(node "/Users/aroman/Work/phantom-perp/.claude/get-shit-done/bin/gsd-tools.cjs" init quick "$DESCRIPTION")
...
```bash
node "/Users/aroman/Work/phantom-perp/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(quick-${quick_id}): ${DESCRIPTION}" --files ${file_list}
```
